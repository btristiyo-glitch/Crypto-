...
def run_scan():
    ...
    for symbol in matched:
        try:
            ...
            try:
                ...
                for item in pools.get("pools", []):
                    ...
                    if token and token == base_symbol:
                        ...
                        fdv_score = estimate_fdv_score(fdv, vol24, liq)
                        buy_surge_score, ratio, buy_share, tx = estimate_buy_surge_from_trending(item)
                        vol_accel_score = estimate_volume_acceleration(vol24, tx24)

                        extra_bits.extend([
                            f"FDV {int(fdv):,}",
                            f"buy ratio {ratio:.2f}",
                            f"buy share {buy_share:.2f}",
                            f"tx24 {int(tx24):,}",
                            f"liq {int(liq):,}",
                        ])
                        break
            except Exception as e:
                log_err(f"Trending enrichment error for {symbol}: {e}")
            ...
            if metrics_by_tf["15m"]["vol20"] < MIN_QUOTE_VOLUME_USDT and not narrative_hit and not early:
                update_blacklist(blacklist, symbol, False)
                continue

            if metrics_by_tf["15m"]["cross_count"] >= 6 and metrics_by_tf["15m"]["sma_gap"] < 0.01 and not narrative_hit:
                update_blacklist(blacklist, symbol, False)
                continue

            levels = build_levels_from_4h(dfs["4H"])
            last_price = round(float(dfs["4H"]["close"].iloc[-1]), 8)

            row = {
                "timestamp": datetime.now().isoformat(timespec="seconds"),
                "symbol": symbol,
                "label": build_signal_label(score, early=early),
                "score": score,
                "rank": rank,
                "reason_for_rank": build_reason(notes, extra_bits),
                "price": last_price,
                "entry": levels["entry"],
                "stop": levels["stop"],
                "tp1": levels["tp1"],
                ...
                "fdv_score": fdv_score,
                "buy_surge_score": buy_surge_score,
            ...
            results.append(row)
            csv_rows.append(row)
            update_blacklist(blacklist, symbol, True)
    ...
    results = sorted(results, key=lambda x: (x["rank"], x["score"]), reverse=True)[:TOP_N]
